...
class Api {
    ...
    func loadRange(pair: Pair, from: Date, until: Date) async throws -> [OHLC] {
        ...
        return try await getTimeSeries(pair: pair, otherParams: "start_date=\(dateformatter.string(from: from))&end_date=\(dateformatter.string(from: until.addingTimeInterval(60*60*24)))&timezone=\(TimeZone.current.identifier)")
    }
    
    private func getTimeSeries(pair: Pair, otherParams: String) async throws -> [OHLC] {
        guard let url = URL(string: "\(Api.endpoint)?\(Api.defaultParams)&symbol=\(pair.symbol)&\(otherParams)&apikey=\(config.apiKey)") else {
